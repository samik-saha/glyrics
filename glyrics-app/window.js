...
window.onload = function() {
	...
	var alwaysOnTop=chrome.storage.local["alwaysOnTop"];
	if (alwaysOnTop === undefined || alwaysOnTop === true){
		$("#keep-on-top-btn").removeClass("display-none");
		chrome.app.window.current().setAlwaysOnTop(true);
	}
	
	$("dropdown ul li.reload").click(function(){
		pingExt();
		$("dropdown input").prop("checked",false);
	});

    $("dropdown ul li.change-style").click(function(){
        dialog.dialog( "open" );
        $("dropdown input").prop("checked",false);
    });
	
	$("dropdown ul li.window-on-top").click(function(){
		var alwaysOnTop=chrome.app.window.current().isAlwaysOnTop();
		chrome.app.window.current().setAlwaysOnTop(!alwaysOnTop);
		$("dropdown ul li.window-on-top .symbol").toggleClass("display-none");
		$("dropdown input").prop("checked",false);
	});
	
	$("dropdown ul li.feedback").click(function(){
		window.open("https://chrome.google.com/webstore/detail/gaana-lyrics-app/paiehomgejkdifojcddmbinmophkibac/reviews");
		$("dropdown input").prop("checked",false);
	});
};
...
chrome.runtime.onMessageExternal.addListener(function(request, sender,
	...
	if (request.msgType === "trackInfo") {
		...
		setHeader(request.artist, request.title);
...
function getLyrics(artist, title, album) {
	...
	if (!artist) {
		...
		/*
		 * the above method will call getLyrics() again after search on
		 * MusicBrainz is complere
		 */
		return;
	}
	...
	if (artist === 'Not Found') {
		mainView.innerHTML = 'Artist not found! Cannot locate lyrics for <b>'
				+ title
				+ '</b>\
            (<a target="_blank" href="https://www.google.com/search?q='
				+ title + ' lyrics"><u>Search Google</u></a>).';
		searchOnLiricWiki(title);
		return;
	}

	/* update header, in case clicked a search result */
			...
			.ajax({
				...
				success : function(lyricsData, status) {
					try {
						...
						if (lyrics === 'Not found') {
							mainView.innerHTML = 'Lyrics not found for <b>'
									+ title
									+ '</b> by <b>'
									+ artist
									+ '</b>\
                            (<a target="_blank" href="https://www.google.com/search?q='
									...
									+ ' '
									+ artist
									+ ' lyrics"><u>Search Google</u></a>).\
                            <br>'
									+ 'Please add lyrics at ' + '<a href="'
									+ songURL
									+ '" target="_blank"><u>LyricWiki</u></a>.'
							searchOnLiricWiki(title);
				...
				success : function(songData, songStatus) {
					...
					if (lyrics.length === 0) {
						...
						document.getElementById('main').innerHTML = lyrics
								+ '<br class="glyrics"><br class="glyrics"><hr class="glyrics">Lyrics provided by <a href="'
			...
			.ajax({
				...
				success : function(resultsPage, songStatus) {
					...
					$(resultsPage)
							...
							.each(
									function(index, element) {
										articleTitle = $(this).children()
												.children('h1').children('a')
												.text();
										articleLink = $(this).children()
												.children('h1').children('a')
												.prop('href');
										...
										if (articleTitle.indexOf(':') > 0) {
											...
											if (songTitle.toLowerCase().search(
													title.toLowerCase()) !== -1) {
												// create <a> element containing
												// the result
												result = document
														.createElement('a');
												result.setAttribute('href',
														articleLink);
												result.setAttribute('target',
														'_blank');
												...
												result.onclick = function() {
													...
													lyricArtist = elementText
															.substr(
																	0,
																	elementText
																			.indexOf(':'));
													lyricTitle = elementText
															.substr(elementText
																	.indexOf(':') + 1);
													getLyricsFromLyricWikiURL(event.srcElement
															.getAttribute('href'));
												};
											}
										}
					...
					if (lwSearchResults.length === 0) {
						searchElement.innerHTML = 'No match found for "'
								+ title + '".';
					} else {
					...
					// If only one result returned and the song title matches
					// exactly, then fetch lyrics from the page immediately
					/*
					 * if (lwSearchResults.length == 1 &&
					 * lwSearchResults[0].innerHTML.substr(lwSearchResults[0].innerHTML.indexOf(':')+1).trim().toLowerCase()
					 * === title.toLowerCase()){ //Set lyricArtist and
					 * lyricTitle to be displayed on header elementText =
					 * lwSearchResults[0].innerHTML; lyricArtist =
					 * elementText.substr(0,elementText.indexOf(':'));
					 * lyricTitle =
					 * elementText.substr(elementText.indexOf(':')+1);
					 * 
...
function getArtistFromMusicBrainz(title, album) {
	...
	query = (!album) ? 'recording:"' + title + '"' : 'recording:"' + title
			+ '" AND release:"' + album + '"';
	...
	$
			.ajax({
				...
				error : function(jqXHR, textStatus, errorThrown) {
					...
					mainView.innerHTML = 'An error occurred while searching artist on MusicBrainz for "'
							+ title + '". Please retry.';
				},
